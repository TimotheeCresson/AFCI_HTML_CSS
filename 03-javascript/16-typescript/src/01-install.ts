"use strict"
/*
    Typescript est une surcouhe à Javascript
    C'est à dire que toute ce qui est faisable en JS, est faisable en Typescript.
    ...
    *   (npx) tsc pathToFile.ts   donc ici   npx tsc src/01-install.ts

    Il est de bon ton de séparer les fichiers TS et JS :
    *   (npx) tsc pathToFile.ts --outDir folderName   donc ici  npx tsc ./src/01-install.ts --outDir dist           dist est ou je veux le mettre

    Pour éviter de retaper tout cela, on peut créer à la racine de notre projet, un fichier nommée :
    *   tsconfig.json
...
//ou
// const btn =<HTMLElement> document.querySelector("#compte");
let i = 0;
