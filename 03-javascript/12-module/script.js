"use strict";
/*
    Ecmascript permet l'export et import de fonctions et objets (entre autres choses)
    Cela va permettre de diviser nos projets en plusieurs fichiers réutilisables
    Sur un gros projet avec des tas de fichiers, cela évite de devoir trouver dans quels ordres tels ou tels fichiers doit être placé.
    ...
    Devant la fonction, l'objet, la classe que l'on souhaite exporter, nous allons ajouter l'un de ces mots clefs: 
    ...
    Sans "export default" dans le fichier exporté, il suffit d'utiliser le mot clef "import", suivi d'entre accolade, séparés de virgules, les éléments que l'on veut importer.
    ...
    Si un "export default" est présent, on pourra placer avant les accolades, un nom (peu importe lequel) qui servira de variable contenant l'élément importé par défaut 
        *  import biche, {salut, coucou} from "./salutation.js";
    ...
    Si un nom est trop long, ou bien si vous importez plusieurs fichiers contenant des éléments de même nom, il est possible de renommé un import avec le mot clef "as" suivi du nouveau nom :
        * import biche, {salut, coucou as c} from "./salutation.js";
    ...
    Si du code exécutable se trouve dans le fichier importé, il sera executé au premier import (et le premier seulement)
*/

import chaussette, {salut, coucou as c} from "./salutation.js";  // chaussette et notre export default, on peut le renommé directement et appelé sa fonction
salut();
/*
...
*/
c("Maurice"); // on a renommé plus haut coucou as c
chaussette();  // Ici il s'agit de la fonction bonjour()
...
/*
    Parfois, importer directement tous les fichiers dont on aura besoin à l'avenir, peut être lourd.
...
async function hello() {
    ...
    console.log(hey);
    /*
        La fonction import retourne une promesse, et une fois réalisé, elle nous rend un objet contenant tous nos exports
    */
   hey.default();
   hey.coucou("Germaine");
   hey.salut();
