/*
    ...
    Son but est d'accueillir des éléments HTML qui vont servir à être récupéré par Javascript afin d'être cloné et réutilisé à divers endroits.

    On commencera par sélectionné le template qui nous intéresse.
    Puis avec la propriété "content", on récupére le contenu du template sous la forme d'un "documentFragment".

    Enfin, nous clonerons ce fragment, via la méthode ".cloneNode(true)" dont le boolean permet de cloner son contenu en entier
...
// Je sélectionne les différents éléments que je souhaite modifier : 
...
async function getBlog() {
    ...
    articles.forEach(a=>{
        ...
        // Et j'insère le clone dnas l'HTML :
...
/*
    Si les templates sont utilisables seul, les slots eux accompagnent forcément le shadowDOM
