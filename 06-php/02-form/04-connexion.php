<?php 
/* 
    Pour savoir d'une page à l'autre que notre utilisateur est connecté, je vais garder l'information en session.
*/
session_start();
/* 
    Si l'utilisateur est déjà connecté, je le redirige ailleurs
*/
...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['login'])) {
    ...
    if (empty($error)) {
        /* 
            Habituellement, il nous faudrait se connecter à la BDD (base de donnée) et envoyer une requête pour trouver l'utilisateur.
            ...
            json_decode() permet de transformer du json en données PHP (on ne l'utilisera pas ici mais json_encode() transforme des données PHP en JSON)
        */
        $users = file_get_contents("../ressources/users.json");
        $users = json_decode($users, true);  // true pour me le donner en tableau associatif (si on met false, il ne me le donne pas en tableaux associatif)
        ...
        if ($user) {
            /* 
                La fonction "password_verify()" permet de vérifier si un mot de passe non hashé correspond à un mot de passe hashé (voir dans le dossier json)
            */
            ...
            // pour tester connexion basilic@laposte.net  avec mdp : detective
    }
}
