 ...
 * @return void
 */
function shouldBeLogged(bool $logged = true, string $redirect = "/"): void { //void est utilisé pour indiquer qu'une fonction ne retourne aucune valeur. Cela signifie que la fonction exécute des actions ou des opérations, mais elle ne produit pas de résultat qui doit être utilisé ou manipulé par le code appelant.
    if ($logged) {
...
//  *  $name : Le nom du cookie (dans ce cas, "PHPSESSID").
// $value : La valeur du cookie (dans ce cas, une chaîne vide "" pour effacer la valeur existante).
// $expire : La date d'expiration du cookie. En utilisant time() - 3600, cela définit l'expiration à une heure avant l'heure actuelle, ce qui conduit à la suppression immédiate du cookie. 
?>
