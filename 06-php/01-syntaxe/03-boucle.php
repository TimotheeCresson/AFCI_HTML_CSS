...
/* 
    ...
    La 1ère expression est évalué avant de commencer la boucle.
    La snde est évalué au début de chaques itérations et continuera la boucle si "true"
    La 3ème est évalué à chque fin d'itération.
*/
for($y = 0; $y < 5; $y++) {
    echo $y;
    echo "<br>";
...
/* 
    foreach permet d'itérer sur un tableau, il se structure ainsi :
        foreach($tableau as $nouvelleVariable) {
            instruction à répéter
        }
    foreach répète les instructions pour chaque élément du tableau changeant à chaque fois la valeur de la variable par l'élément suivant du tableau.
*/
foreach($a as $food) {
    echo "$food <br>";
}
/* 
    Il est aussi possible de récupérer l'index ou la clef (dans le cas d'un tableau associatif) avec cette structure :
        foreach($tableau as $variable1 => $variableValeur) {
            instruction à répéter
        }
*/
foreach($a as $k => $food) {
...
/* 
    Toutes les boucles peuvent utiliser les mots clefs "continue" et "break"
        - "continue" met fin à l'itération actuelle et passe à la suivante
        - "break" met fin à la boucle
*/
foreach($a as $f) {
    if ($f === "spaghetti") continue;   // affiche thon
