...
UPDATE article SET PRIX_ACHAT = PRIX_ACHAT * 1.1 WHERE ID_TYPE = (SELECT ID_TYPE FROM type WHERE NOM_TYPE = "Trappiste")
 and ID_Couleur = (SELECT ID_Couleur FROM couleur WHERE NOM_COULEUR = "Blonde");
...
SET a.TITRAGE = (
  ...
  FROM article a2
  WHERE a2.ID_TYPE = a.ID_TYPE AND a2.ID_Couleur = a.ID_Couleur
    AND a2.TITRAGE IS NOT NULL
)
